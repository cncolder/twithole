...
class TwitHole
  ...
  def call(env)
    ...
    self.class.log.unshift({:time => Time.now.strftime('%Y-%m-%d %H:%M:%S'), :method => request_method, :url => twitter_uri.path, :result => twitter_response.msg}) if twitter_uri.path !~ /favicon\.ico$/
